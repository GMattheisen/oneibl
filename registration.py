...
class RegistrationClient:
    ...
    def register_session(self, ses_path, file_list=True):
        ...
        try:
            ...
            _logger.error(f"User: {md['PROTOCOL_CREATOR'][0]} doesn't exist in Alyx. ABORT")
            raise e
...
def _read_settings_json_compatibility_enforced(json_file):
    ...
    if version.le(md['IBLRIG_VERSION_TAG'], '3.2.3'):
        ...
        if 'weighings' in md['SUBJECT_EXTRA'].keys():
            md['SUBJECT_EXTRA'].pop('weighings')
        if 'water_administration' in md['SUBJECT_EXTRA'].keys():
            md['SUBJECT_EXTRA'].pop('water_administration')
...
def _get_session_performance(md, ses_data):
    ...
    # task specific logic
    if 'habituationChoiceWorld' in md['PROTOCOL']:
        n_correct_trials = 0
    else:
